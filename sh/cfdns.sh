...
action=$1
if [[ $action = "d" ]]; then
read -e -p "(网络检测地址: ):" HOST
        [[ -z "HOST" ]] && HOST=$HOST
read -e -p "(ip更新域名: ):" NAME
        [[ -z "NAME" ]] && NAME=$HOST
fi

ping -c 4 -W 5 $HOST >/dev/null
if [  $? -eq 0  ];then
echo "`date "+%m-%d %H:%M" ` | 网络正常"
else

set -o errexit
set -o nounset
set -o pipefail
CFKEY=4d399146125f4b27a6dee53abdf4baaa5eb3a
CFUSER=scenmentga@gmail.com
CFZONE_NAME=onon.cloud
CFRECORD_NAME=$NAME
CFRECORD_TYPE=A
CFTTL=120
FORCE=false
...
if [ "$RESPONSE" != "${RESPONSE%success*}" ] && [ "$(echo $RESPONSE | grep "\"success\":true")" != "" ]; then
  echo " `date "+%m-%d %H:%M:%S" ` | Updated succesfuly!"
